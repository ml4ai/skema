...
async def get_lines_of_model(zip_file: UploadFile = File()) -> List[Dynamics]:
    ...
    # read in the code, for the prompt
        ...
        start_line_begin = response_zip.json()['modules'][0]['metadata_collection'][3][0]['line_begin']

        if file_line_begin != start_line_begin:
            block.append(f"L{file_line_begin}-L{start_line_begin}")
