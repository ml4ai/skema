...
class HealthStatus(BaseModel):
    ...
    mathjax: int = Field(description="HTTP status code for mathjax service (used by eqn2mml latex2mml endpoints)", ge=100, le=599)
    eqn2mml: int = Field(description="HTTP status code for eqn2mml service (img2mml endpoints)", ge=100, le=599)
    code2fn: int =Field(description="HTTP status code for code2fn service", ge=100, le=599) 
