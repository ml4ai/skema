...
def annotate_text_with_skema(text: Union[str, List[str]]) -> List[Dict[str, Any]]:
    ...
    if isinstance(text, str):
        payload = [
            text]  # If the text to annotate is a single string representing the contents of a document, make it a list with a single element
    else:
...
def annotate_text_with_mit(texts: Union[str, List[str]]) -> List[Dict[str, Any]]:
    ...
    if isinstance(texts, str):
        texts = [
            texts]  # If the text to annotate is a single string representing the contents of a document, make it a list with a single element
...
def normalize_extractions(
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        if arizona_extractions and mit_extractions:
            ...
            response = requests.post(f"{MIT_TR_ADDRESS}/integration/get_mapping", params=params, data=data)
...
@router.post("/integrated_text_extractions", summary="Posts one or more plain text documents and annotates with SKEMA and/or MIT text reading pipelines")
