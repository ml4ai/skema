...
contact = {
  "name": "SKEMA team",
  "url": "https://github.com/ml4ai/skema/issues"
}
...
app = FastAPI(version=VERSION, title="SKEMA API", description=description, contact=contact, openapi_tags=tags_metadata)
...
@app.get("/healthcheck", tags=["core"], summary="Health of component services", status_code=status.HTTP_200_OK, responses={
  200: {
...
async def healthcheck(response: Response) -> schema.HealthStatus:
  ...
  status_code = status.HTTP_200_OK if all(code == 200 for code in [morae_status, mathjax_status, eqn2mml_status, code2fn_status]) else status.HTTP_500_INTERNAL_SERVER_ERROR
