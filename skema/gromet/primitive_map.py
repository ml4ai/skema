...
class UAdd(object):
    ...
    def exec(
        operand: Union[int, float, complex]
...
class Add(object):
    ...
    def exec(
        augend: Union[int, float, complex], addend: Union[int, float, complex]
    ) -> Union[int, float, complex]:
...
class Sub(object):
    ...
    def exec(
        minuend: Union[int, float, complex],
        subtahend: Union[int, float, complex],
    ) -> Union[int, float, complex]:
...
class Mult(object):
    ...
    def exec(
        multiplier: Union[int, float, complex],
        multiplicand: Union[int, float, complex],
    ) -> Union[int, float, complex]:
...
class Pow(object):
    ...
    def exec(
        base: Union[int, float, complex], power: Union[int, float, complex]
    ) -> Union[int, float, complex]:
        return base**power
...
class LShift(object):
    ...
    def exec(
        operand1: Union[int, float, complex],
        operand2: Union[int, float, complex],
    ) -> Union[int, float, complex]:
        return operand1 << operand2
...
class BitOr(object):
    ...
    def exec(
        binary1: Union[int, float, complex],
        binary2: Union[int, float, complex],
    ) -> Union[int, float, complex]:
...
class Lt(object):
    ...
    def exec(
        number1: Union[int, float, complex],
        number2: Union[int, float, complex],
    ) -> bool:
...
class Lte(object):
    source_language_name = {
        ...
        "GCC": "le_expr",
        "CAST": "Lte",
    }  # TODO: Is it LtE or Lte for Python and CAST
    inputs = [Field("number1", "Number"), Field("number2", "Number")]
    outputs = [Field("result", "Boolean")]
    shorthand = "<="
    documentation = ""
...
class In(
    object
):  # TODO: How should In and NotIn work? What is the difference between in, member, List_in?
...
class del_elm(object):
    ...
    def exec(input_set: set, element: Any) -> set:
        return input_set.remove(
            element
        )  # TODO: Do we need to check if this exists first? Will throw KeyError if it does not
...
class new_List(object):
    ...
    def exec(*elements: Any) -> list:
        return list(
            elements
        )  # Interestingly when passing variable sized arguments, it is passeed as a tuple
...
class Array_get(
    object
):  # TODO: Should this do type checking for each element?
...
class Array_set(object):
    ...
    def exec(
        array_input: numpy.ndarray, index: int, value: Any
    ) -> numpy.ndarray:
...
class Array_new_Iterator(object):
    ...
    ) -> Iterator:  # TODO: Numpy array is not built in type, can we still customize type hints
...
class Tuple_set(object):  # TODO: Should this exist?
    ...
    def exec(tuple_input: tuple, index: int, value: Any) -> tuple:
        placeholder_list = list(
            tuple_input
        )  # tuples are immutable, so conversions must be made
...
class Map_get(object):
    ...
    def exec(
        map_input: dict, index: Any
    ) -> Any:  # TODO: Should index really be Any for dict?
...
class IteratorSet_next(object):
    ...
    outputs = [
        Field("element", "Any"),
        Field("iterator_output", "IteratorSet"),
        Field("stop_condition", "Boolean"),
    ]
...
class IteratorTuple_next(object):
    ...
    outputs = [
        Field("element", "Any"),
        Field("iterator_output", "IteratorTuple"),
        Field("stop_condition", "Boolean"),
    ]
    ...
    def exec(
        iterator_input: Iterator[tuple],
    ) -> Tuple[Any, Iterator[Tuple], bool]:
        current_element = None
        # We have to wrap this code in a try except block because of the call to next()
        # next() will throw an error if you've reached the end of the iterator
        # You can specify a default value for it to return instead of an exception,
...
class IteratorArray_next(object):
    ...
    outputs = [
        Field("element", "Any"),
        Field("iterator_output", "IteratorArray"),
        Field("stop_condition", "Boolean"),
    ]
    ...
    def exec(
        iterator_input: Iterator,
    ) -> Tuple[
        Any, Iterator, bool
    ]:  # TODO: Can we say Iterator[numpy.ndarray]
        current_element = None
        # We have to wrap this code in a try except block because of the call to next()
        # next() will throw an error if you've reached the end of the iterator
        # You can specify a default value for it to return instead of an exception,
...
class IteratorList_next(object):
    ...
    outputs = [
        Field("element", "Any"),
        Field("iterator_output", "IteratorList"),
        Field("stop_condition", "Boolean"),
    ]
    ...
    def exec(
        iterator_input: Iterator[List],
    ) -> Tuple[Any, Iterator[List], bool]:
        current_element = None
        # We have to wrap this code in a try except block because of the call to next()
        # next() will throw an error if you've reached the end of the iterator
        # You can specify a default value for it to return instead of an exception,
...
class IteratorMap_next(object):
    ...
    outputs = [
        Field("element", "Any"),
        Field("iterator_output", "IteratorMap"),
        Field("stop_condition", "Boolean"),
    ]
    ...
    def exec(
        iterator_input: Iterator[dict],
    ) -> Tuple[Any, Iterator[Dict], bool]:
        current_element = None
        # We have to wrap this code in a try except block because of the call to next()
        # next() will throw an error if you've reached the end of the iterator
        # You can specify a default value for it to return instead of an exception,
...
class CASTGenericSet:
    ...
    inputs = [
        Field("indexible_input", "Indexable"),
        Field("index", "Any"),
        Field("value", "Any"),
    ]
...
class CASTGenericNext:
    ...
    outputs = [
        Field("element", "Any"),
        Field("iterator_output", "Iterator"),
        Field("stop_condition", "Boolean"),
    ]
    shorthand = "next"
    documentation = "The cast currently uses generic primitive operators (_get, _set, iter, next) while the  Gromet uses specific operators (IteratorMap_next). These primitive ops are a tempory fix for that mismatch"
...
class Is(object):
    source_language_name = {
        "Python": "is",
        "CAST": "Is",
    }  # TODO: Should Python/CAST be Is or is?
    inputs = [Field("operand1", "Any"), Field("operand2", "Any")]
    outputs = [Field("result", "Boolean")]
    shorthand = "is"
    documentation = ""
...
class NotIs(object):
    source_language_name = {
        "Python": "is not",
        "CAST": "NotIs",
    }  # TODO: What should Python name be here?
    inputs = [Field("operand1", "Any"), Field("operand2", "Any")]
    outputs = [Field("result", "Boolean")]
    shorthand = "not is"
    documentation = ""
...
class IsInstance(object):
    ...
    inputs = [
        Field("operand", "Any"),
        Field("type", "Type"),
    ]  # TODO: Get confirmation on adding Type field
    outputs = [Field("result", "Boolean")]
    shorthand = "type"
    documentation = ""

    def exec(
        operand: Any, type: type
    ) -> bool:  # TODO: Fix name of right argument
...
def get_class_obj(
    op: str, language: str, debug=False
) -> Any:  # TODO: Update the type hints for this
    ...
    try:
        ...
        if debug:
            print(
                f"Operation not supported: {op} for language {language} ... Returning None"
            )
        return None
...
def get_shorthand(op: str, language: str) -> str:
    ...
    if class_obj:
        try:
            ...
            print(
                f"Operation has no shorthand: {op} for language {language} ... Returning None"
            )
...
def get_inputs(op: str, language: str) -> str:
    ...
    if class_obj:
        try:
            ...
            print(
                f"Operation has no inputs: {op} for language {language} ... Returning None"
            )
...
def get_outputs(op: str, language: str, debug=True) -> str:
    ...
    if class_obj:
        try:
            ...
            print(
                f"Operation has no outputs: {op} for language {language} ... Returning None"
            )
...
primitive_ops = inspect.getmembers(
...
for class_name, class_obj in primitive_ops:
    if hasattr(class_obj, "source_language_name"):
        if "Python" in class_obj.source_language_name:
            python_primitive_dict[
                class_obj.source_language_name["Python"]
            ] = class_obj
        if "GCC" in class_obj.source_language_name:
            gcc_primitive_dict[
                class_obj.source_language_name["GCC"]
            ] = class_obj
        if "CAST" in class_obj.source_language_name:
            cast_primitive_dict[
                class_obj.source_language_name["CAST"]
            ] = class_obj

primitive_map = {
    "Python": python_primitive_dict,
