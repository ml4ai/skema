...
from skema.gromet.metadata import Metadata, SourceCodeReference
...
def get_module_metadata(module: GrometFNModule, idx: int) -> List[Metadata]:
    ...
    if idx < len(module.metadata_collection):
        ...
        raise Exception(
            f"GrometFNModule {module.name} as metadata_collection size {len(module.metadata_collection)}, "
            f"but asked for index {idx}"
        )


def find_source_code_reference(
    module: GrometFNModule, idx: int
) -> SourceCodeReference:
    """
        ...
        if fn.pof:
            for output_port in fn.pof:
                if output_port.name:
                    ...
                    box = fn.bf[
                        output_port.box - 1
                    ]  # the box (call) the output_port is attached to
                    ...
                    if box.metadata:
                        ...
                        source_code_reference = find_source_code_reference(
                            module, box.metadata - 1
                        )

                    # Now see if there is a LiteralValue assigned directly to this output_port
                    if box.contents:
                        attribute_contents = module.attributes[
                            box.contents - 1
                        ]  # the contents of the box-call
                        if attribute_contents.type == "FN":
                            ...
                            if fn_contents.b[0].function_type == "EXPRESSION":
                                ...
                                if fn_contents.wfopo:
                                    expr_box_idx = fn_contents.wfopo[
                                        0
                                    ].tgt  # identify the value source of the output port
                    ...
                    named_output_ports.append(
                        (name, value, source_code_reference)
                    )
...
EXP0_GROMET_JSON = os.path.join(
    ROOT, "gromet/examples/exp0/FN_0.1.4/exp0--Gromet-FN-auto.json"
)
EXP1_GROMET_JSON = os.path.join(
    ROOT, "gromet/examples/exp1/FN_0.1.4/exp1--Gromet-FN-auto.json"
)
EXP2_GROMET_JSON = os.path.join(
    ROOT, "gromet/examples/exp2/FN_0.1.4/exp2--Gromet-FN-auto.json"
)
CHIME_SIR_GROMET_JSON = os.path.join(
    ROOT,
    "epidemiology/CHIME/CHIME_SIR_model/gromet/FN_0.1.4/CHIME_SIR--Gromet-FN-auto.json",
)
EXAMPLE_GROMET_JSON_FILE = (
    "../../../notebooks/gromet/CHIME_SIR_while_loop--Gromet-FN-auto.json"
)
