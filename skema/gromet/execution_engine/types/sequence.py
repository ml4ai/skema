...
class Sequence_get(object):
    ...
    inputs = [
        Field("sequence_input", "Sequence"),
        Field("index", "DimensionalIndex"),
    ]
...
class Sequence_concatenate(object):
    ...
    def exec(*sequence_inputs: Sequence) -> Sequence:
        ...
        assert type(
            sequence_inputs[0] != range
        )  # Range type doesn't support concatenation
        assert all(
            isinstance(sequence, type(sequence_inputs[0]))
            for sequence in sequence_inputs
        )  # Cannot concatenate sequences of different types

        if isinstance(sequence_inputs[0], numpy.ndarray):
            Sequence_concatenate.Array_concatenate(sequence_inputs)
        else:
            return type(sequence_inputs[0])(
                itertools.chain.from_iterable(sequence_inputs)
            )

    def Array_concatenate(
        array_inputs: Tuple[numpy.ndarray, ...]
    ) -> numpy.ndarray:
...
class Sequence_replicate(object):
    ...
    def Array_replicate(
        array_input: numpy.ndarray, count: int
    ) -> numpy.ndarray:
