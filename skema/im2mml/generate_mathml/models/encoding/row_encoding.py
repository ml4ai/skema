...
class RowEncoding(torch.nn.Module):
    ...
    def __init__(self, device, dec_hid_dim, dropout):
        ...
        self.lstm = torch.nn.LSTM(512, dec_hid_dim, num_layers=1, dropout=dropout, bidirectional=False, batch_first=False)
    ...
    def forward(self, enc_output):
        ...
        for wh in range(0, enc_output.shape[2]):
            ...
            position_vector = torch.Tensor(row.shape[1]).long().fill_(wh).to(self.device) # [batch]
            # self.emb(pos) ==> [batch, 512]
            lstm_input = torch.cat((self.emb(position_vector).unsqueeze(0), row), dim = 0) # [W+1, batch, 512]
        ...
        final_output = torch.cat(outputs, dim =0)  #[H, W+1, BATCH, dec_hid_dim]
        # modifying it to [H*W+1, batch, dec_hid_dim]
        final_output = final_output.view( final_output.shape[0]*final_output.shape[1],
                                        final_output.shape[2], final_output.shape[3])
