...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, encoder_outputs, hidden):
        ...
        if self.n_layers==1:
            hidden = hidden.repeat(src_len, 1, 1).permute(1,0,2)           # (B,L,dec_hid_dim)
        else:
            """
            ...
            """
            hidden = hidden.repeat(src_len, 1, 1).permute(1,0,2)           # (B,n*L,dec_hid_dim)
            encoder_outputs = encoder_outputs.repeat(1, self.n_layers, 1) # (B, n*L, dec_hid_dim)
        ...
        alpha = F.softmax(attention, dim=1).unsqueeze(0)        #[1, batch size, src len]
        weighted = torch.bmm(alpha.permute(1, 0, 2), encoder_outputs)   # [B, 1, dec_hid_dim]
