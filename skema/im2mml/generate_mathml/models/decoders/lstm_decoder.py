...
class LSTM_Decoder(nn.Module):
    ...
    def __init__(self, embed_dim, encoder_dim, dec_hid_dim, output_dim, n_layers, device, dropout, tfr):
        """
        ...
        self.attention = BahdanauAttention(encoder_dim, dec_hid_dim, n_layers)  # attention network
        ...
        self.lstm = nn.LSTM(embed_dim, dec_hid_dim, num_layers=n_layers, dropout=dropout, bias=True)  # decoding LSTMCell
        self.fc = nn.Linear(dec_hid_dim, output_dim)  # linear layer to find scores over vocabulary
    ...
    def initialize_hidden(self, batch_size, hid_dim, n_layers):
        ...
        """
        hidden = torch.zeros(batch_size, hid_dim).unsqueeze(0)  # [1, batch, hid_dim]
        hidden = hidden.repeat(n_layers, 1, 1)  # (N, batch, dec_hid_dim)
        return  hidden

    def forward(self, enc_output, trg, is_train, is_test, encoding_type, max_len, sos_idx, batch_size ):
        """
        param enc_output: [B, L, dec_hid_dim]
        param trg: [B, max_len]
        """
        outputs = torch.zeros(max_len, batch_size, self.output_dim).to(self.device)      # (max_len, B, output_dim)
        hidden = cell = self.initialize_hidden(batch_size, self.dec_hid_dim, self.n_layers).to(self.device)    # (1, B, dec_hid_dim)
        ...
        for i in range(1,max_len):
            # Embedding
            embeddings = self.embedding(token.unsqueeze(0))  # (1, batch_size, embed_dim)
            # Calculate attention

            final_attn_encoding,alpha = self.attention(enc_output, hidden)    # [ 1, B, dec_hid_dim]

            # lstm input
            lstm_input = torch.cat((embeddings, final_attn_encoding), dim=2)    # [1, B, dec_hid_dim+embed]
            lstm_input = self.lstm_input_layer(lstm_input)  # [1, B, embed_dim]
            lstm_output, (hidden, cell) = self.lstm(lstm_input, (hidden, cell))    # H: [1(D*N), B, hid]     O: [1, B, Hid*D(=1)]
