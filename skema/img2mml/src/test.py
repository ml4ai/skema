        ...
        for i, (img, mml) in enumerate(test_dataloader):
            ...
            if is_test:
                ...
                if model_type=="opennmt":
                    outputs_reshaped = outputs[:,1:,:].contiguous().view(-1, output_dim)    # (B * max_len-1, output_dim)
                elif model_type=="cnn_xfmer":
                    outputs_reshaped = outputs.contiguous().view(-1, output_dim)    # (B * max_len-1, output_dim)
            ...
            if is_test:
                for idx in range(batch_size):
                    ...
                    if beam_params is not None:
                        beam_k, alpha, min_length_bean_search_normalization = beam_params
                        predicted_seq = beam_search(outputs[idx,:,:], beam_k, alpha,
                                            min_length_bean_search_normalization) # list of all eqns and score
