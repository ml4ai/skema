...
def main(year):
    for month_dir in directories:
        ...
        tex_files = os.path.join(base_dir, "tex_files_fonts")
...
def create_tex_doc(eqn, keyword_dict, keyword_macro_dict, tex_folder, tex_name):
    ...
    declare_math_operator_in_eqn = [kw for kw in keyword_dict.keys() if kw in eqn]
...
def template(eqn, preamble_dmo, preamble_macro):
    ...
    selected_font = random.choice(
        math_fonts
    )  # randomly select one math font to generate pngs
    if selected_font != "":
        temp1 = (
            "\\documentclass{standalone}\n"
            "\\usepackage{amsmath}\n"
            "\\usepackage{amssymb}\n"
            f"\\usepackage{{{selected_font}}}\n"
        )
    else:
        temp1 = (
            "\\documentclass{standalone}\n"
