...
def main(year):
    ...
    for month_dir in directories:
        ...
        latex_images = os.path.join(path, "latex_images_fonts")
...
def pool_path(path, logger):
    ...
    tex_folder_path = os.path.join(path, "tex_files_fonts")

    for folder in os.listdir(tex_folder_path):

        # make results PNG directories
        pdf_dst_root = os.path.join(path, f"latex_images_fonts/{folder}")
