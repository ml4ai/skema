...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, encoder_outputs, hidden):
        ...
        if self.n_layers == 1:
            ...
            """
            we need to repeat the hidden to src_len 
            that will result in hid:(n*L, B, dec_hid_dim)
            Hence we need to repeat encoder_outputs too, 
            to match hidden size i.e. hid_src_len-src_len
            """
            hidden = hidden.repeat(src_len, 1, 1).permute(
                1, 0, 2
            )  # (B,n*L,dec_hid_dim)
