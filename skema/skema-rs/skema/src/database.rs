...
fn create_module(gromet: &ModuleCollection) -> Vec<String> {
    ...
    let name = format!(
        "name:{:?}",
        gromet.modules[0].r#fn.b[0].name.as_ref().unwrap()
    );
    let metadata_idx = gromet.modules[0].r#fn.b[0].metadata.as_ref().unwrap();
                ...
                if function_call_repeat {
                    ...
                    let n1 =
                        Node {
                            n_type: String::from("Function"),
                            value: None,
                            name: Some(eboxf.value.b[0].name.clone().map_or_else(
                            ...
                            if node.n_type != "Opi" {
                                ...
                                for p in node.in_indx.as_ref().unwrap().iter() {
                                    ...
                                    if (wire.src as u32) == *p {
                                        ...
                                        if node.node_id.clone() == "n211" {
                                            println!("src_idx: {}", src_idx);
                                            println!("src_pif: {:?}", src_pif);
                                            println!("src: {:?}", node.node_id.clone());
                                            println!(
                                                "src_node.att_bf_idx: {}",
                                                node.att_bf_idx.clone()
                                            );
                                        }
                                    }
                                }
                        ...
                        if !node.out_idx.is_none() {
                            ...
                            if node.n_type != "Opo" {
                                ...
                                for p in node.out_idx.as_ref().unwrap().iter() {
                                    ...
                                    if (wire.tgt as u32) == *p {
                                        wff_src_tgt.push(node.node_id.clone());
                                        if node.node_id.clone() == "n204" {
                                            println!("tgt: {:?}", node.node_id.clone());
                                            println!(
                                                "tgt_node.att_bf_idx: {}",
                                                node.att_bf_idx.clone()
                                            );
                                            println!("tgt_idx: {}", tgt_idx);
                                            println!("tgt_pof: {:?}", tgt_pof);
                                            println!("internal wff wire: {:?}", wff_src_tgt);
                                        }
                                    }
                                }
                            }
