...
from CAST2GrFN.model.cast import (
    AstNode,
    Assignment,
    Attribute,
    BinaryOp,
...
from .modules_list import BUILTINS, find_std_lib_module
...
class PyASTToCAST():
    ...
    def visit_BoolOp(self, node: ast.BoolOp, prev_scope_id_dict: Dict, curr_scope_id_dict: Dict):
        ...
        [self.filenames[-1], node.col_offset, node.end_col_offset, node.lineno, node.end_lineno]
    ...
    def visit_Subscript(self, node: ast.Subscript, prev_scope_id_dict: Dict, curr_scope_id_dict: Dict):
        ...
        # 'Visit' the slice
        slc = node.slice
        f"generated_index_{self.var_count}"
        self.var_count += 1
