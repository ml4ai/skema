...
class AnnCastOperator(AnnCastNode):
    ...
    def to_dict(self):
        result = super().to_dict()
        result["source_language"] = str(self.source_language)
        result["interpreter"] = str(self.interpreter)
    ...
    def equiv(self, other):
        if not isinstance(other, AnnCastOperator):
            return False
        return self.to_dict() == other.to_dict()

    def __str__(self):
        return Operator.__str__(self)
