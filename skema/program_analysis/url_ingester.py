...
def process_git_repo(repo_url: str, checkout_ref=None, write_to_file=False, original_source=False) -> GrometFNModuleCollection:
    """
    ...
    """
    # Calculate the system name from the repo url
    system_name = Path(repo_url).stem
 
    # Create a temporary directory
    with tempfile.TemporaryDirectory() as temp:
        cloned_path = Path(temp) / Path(system_name)
        try:
        ...
        gromet_collection = easy_process_file_system(system_name, str(cloned_path), write_to_file, original_source)
...
def main():
    ...
    process_git_repo(args.repo_url, args.ref, True, args.source)
