...
from skema.model_assembly.metadata import LambdaType
...
class ToGrfnPass:
    def __init__(self, pipeline_state: PipelineState):
        ...
        self.pipeline_state.grfn = GroundedFunctionNetwork(
            grfn_uid,
            identifier,
            timestamp,
            self.network,
    ...
    def create_interface_node(self, lambda_expr):
        ...
        interface_node = LambdaNode(
            lambda_uuid, lambda_type, lambda_str, lambda_func, lambda_metadata
        )
    ...
    def create_loop_top_interface(self, lambda_expr):
        ...
        interface_node = LoopTopInterface(
            lambda_uuid, lambda_type, lambda_str, lambda_func, lambda_metadata
        )
    ...
    def add_grfn_edges(
        self, inputs: typing.List, lambda_node, outputs: typing.List
    ):
        """Parameters:
    ...
    def create_condition_node(
        self, condition_in, condition_out, lambda_expr, subgraph: GrFNSubgraph
    ):
        lambda_uuid = GenericNode.create_node_id()
        lambda_str = lambda_expr
        lambda_func = load_lambda_function(lambda_str)
    ...
    def visit_grfn_assignment(
        self, grfn_assignment: GrfnAssignment, subgraph: GrFNSubgraph
    ):
        ...
        assignment_node.function = load_lambda_function(
            assignment_node.func_str
        )
        ...
        if isinstance(assignment_node, (PackNode, UnpackNode)):
            assignment_node.inputs = ",".join(
                list(
                    map(lambda_var_from_fullid, grfn_assignment.inputs.keys())
                )
            )
            assignment_node.output = ",".join(
                list(
                    map(lambda_var_from_fullid, grfn_assignment.outputs.keys())
                )
            )
    ...
    def visit_node_list(
        self, node_list: typing.List[AnnCastNode], subgraph: GrFNSubgraph
    ):
    ...
    def visit_assignment(
        self, node: AnnCastAssignment, subgraph: GrFNSubgraph
    ):
    ...
    def visit_call(self, node: AnnCastCall, subgraph: GrFNSubgraph):
        ...
        if len(node.top_interface_in) > 0:
            top_interface = self.create_interface_node(
                node.top_interface_lambda
            )
            self.network.add_node(top_interface, **top_interface.get_kwargs())

            inputs = self.grfn_vars_from_fullids(
                node.top_interface_in.values()
            )
            outputs = self.grfn_vars_from_fullids(
                node.top_interface_out.values()
            )
        ...
        if len(node.bot_interface_in) > 0:
            bot_interface = self.create_interface_node(
                node.bot_interface_lambda
            )
            self.network.add_node(bot_interface, **bot_interface.get_kwargs())

            inputs = self.grfn_vars_from_fullids(
                node.bot_interface_in.values()
            )
            outputs = self.grfn_vars_from_fullids(
                node.bot_interface_out.values()
            )
    ...
    def visit_call_grfn_2_2(self, node: AnnCastCall, subgraph: GrFNSubgraph):
        ...
        metadata = create_container_metadata(
            node.func_def_copy.grfn_con_src_ref
        )
        nodes = []
        parent_str = parent.uid if parent is not None else None
    ...
    def visit_function_def(
        ...
        if self.pipeline_state.GENERATE_GRFN_2_2 and not is_func_def_main(
            node
        ):
            return
        ...
        basename = (
            node.name.name
        )  # was originally assigned to 'scope', changed to node.name.name
    ...
    def visit_loop(self, node: AnnCastLoop, subgraph: GrFNSubgraph):
        ...
        basename_id = int(
            node.con_scope[-1][4:]
        )  # stripping out the word 'loop' to obtain a numerical ID
        subgraph = GrFNLoopSubgraph(
            uid,
            ns,
            scope,
            basename,
        ...
        if len(node.top_interface_initial) > 0:
            ...
            grfn_initial = self.grfn_vars_from_fullids(
                node.top_interface_initial.values()
            )
            # collect updated GrFN VariableNodes
            grfn_updated = self.grfn_vars_from_fullids(
                node.top_interface_updated.values()
            )
    ...
    def visit_model_break(
        self, node: AnnCastModelBreak, subgraph: GrFNSubgraph
    ):
        pass

    @_visit.register
    def visit_model_continue(
        self, node: AnnCastModelContinue, subgraph: GrFNSubgraph
    ):
        pass
    ...
    def visit_model_if(self, node: AnnCastModelIf, subgraph: GrFNSubgraph):
        ...
        basename_id = int(
            node.con_scope[-1][2:]
        )  # stripping out the word 'if' to obtain a numerical ID
        subgraph = GrFNSubgraph(
            uid,
            ns,
            scope,
        ...
        if len(node.decision_in) > 0:
            self.create_decision_node(
                node.decision_in,
                node.decision_out,
                condition_var,
                node.decision_lambda,
                subgraph,
            )
    ...
    def visit_model_return(
        self, node: AnnCastModelReturn, subgraph: GrFNSubgraph
    ):
